...
use super::*;
use crate::commitments::{CommitmentStatus, ATTESTOR_VOTE_TIMEOUT_SECONDS};
...
fn test_create_and_get_commitment_success() {
    ...
    let commitment_id = client.create_commitment(&issuer, &counterparty, &terms_hash, &due_at, &Vec::new(&env), &0);
...
fn test_create_commitment_requires_auth() {
    ...
    client.create_commitment(&issuer, &counterparty, &terms_hash, &due_at, &Vec::new(&env), &0);
}
...
fn test_create_commitment_fails_if_due_at_in_past() {
    ...
    let res = client.try_create_commitment(&issuer, &counterparty, &terms_hash, &due_at, &Vec::new(&env), &0);
...
fn test_sequential_unique_ids() {
    ...
    let id1 = client.create_commitment(&issuer, &counterparty, &terms_hash1, &2000, &Vec::new(&env), &0);
    let id2 = client.create_commitment(&issuer, &counterparty, &terms_hash2, &3000, &Vec::new(&env), &0);
...
fn test_dispute_requires_auth() {
    ...
    env.mock_all_auths();
    let id = client.create_commitment(&issuer, &counterparty, &terms_hash, &2000, &Vec::new(&env), &0);
    client.attest(&issuer, &id, &CommitmentStatus::Fulfilled);
...
fn test_reputation_increments_direct_attestation() {
    ...
    let id1 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[1u8; 32]), &due_at, &Vec::new(&env), &0);
    ...
    let id2 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[2u8; 32]), &due_at, &Vec::new(&env), &0);
    ...
    let id3 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[3u8; 32]), &due_at, &Vec::new(&env), &0);
...
fn test_reputation_not_incremented_when_disputed() {
    ...
    env.ledger().with_mut(|l| l.timestamp = 1000);
    let id = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[1u8; 32]), &2000, &Vec::new(&env), &0);
...
fn test_reputation_aggregates_multiple_commitments() {
    ...
    let id1 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[1u8; 32]), &2000, &Vec::new(&env), &0);
    client.attest(&issuer, &id1, &CommitmentStatus::Fulfilled);
    
    // Comm 2: Late (disputed, resolved as Late)
    let id2 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[2u8; 32]), &2000, &Vec::new(&env), &0);
    ...
    let id3 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[3u8; 32]), &2000, &Vec::new(&env), &0);
    client.attest(&issuer, &id3, &CommitmentStatus::Breached);
    
    // Comm 4: Fulfilled (direct)
    let id4 = client.create_commitment(&issuer, &counterparty, &BytesN::from_array(&env, &[4u8; 32]), &2000, &Vec::new(&env), &0);
...
// -----------------------------------------------------------------------------
// Phase 6 - M-of-N Attestor Voting (Issue 62)
// -----------------------------------------------------------------------------

fn setup_voting_test() -> (
    Env,
    RegistryContractClient<'static>,
    Address,
    Address,
    ...
    (env, client, issuer, counterparty, attestors)
}

fn create_voting_commitment(
    env: &Env,
...
#[test]
fn test_create_commitment_with_attestors_stores_state() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();

    env.ledger().with_mut(|l| l.timestamp = 1000);
    let id = create_voting_commitment(&env, &client, &issuer, &counterparty, &attestors, 2000);

    let commitment = client.get_commitment(&id);
    assert_eq!(commitment.attestors, attestors);
    assert_eq!(commitment.threshold, 3);
    assert_eq!(commitment.status, CommitmentStatus::Pending);
}

#[test]
fn test_create_commitment_rejects_zero_threshold() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
...
#[test]
fn test_create_commitment_rejects_threshold_above_attestor_count() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
...
#[test]
fn test_create_commitment_rejects_threshold_with_empty_attestors() {
    let (env, client, issuer, counterparty) = setup_test();
...
#[test]
fn test_create_commitment_rejects_duplicate_attestors() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();

    env.ledger().with_mut(|l| l.timestamp = 1000);
    let first = attestors.get(0).unwrap();
    let mut dup = Vec::new(&env);
    dup.push_back(first.clone());
    dup.push_back(first.clone());
    ...
    assert_eq!(res, Err(Ok(Error::DuplicateAttestor.into())));
}

#[test]
fn test_cast_attestor_vote_keeps_pending_below_threshold() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    client.cast_attestor_vote(&a1, &id, &CommitmentStatus::Fulfilled);

    let commitment = client.get_commitment(&id);
    assert_eq!(commitment.status, CommitmentStatus::Pending);

    let tally = client.get_vote_tally(&id);
    assert_eq!(tally.fulfilled, 1);
    assert_eq!(tally.late, 0);
    assert_eq!(tally.breached, 0);
}

#[test]
fn test_threshold_reached_resolves_commitment() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    for i in 0..3 {
        ...
        client.cast_attestor_vote(&a, &id, &CommitmentStatus::Fulfilled);
    }
    ...
    let tally = client.get_vote_tally(&id);
    assert_eq!(tally.fulfilled, 3);
    assert_eq!(tally.late, 0);
    assert_eq!(tally.breached, 0);
}

#[test]
fn test_votes_for_different_outcomes_do_not_resolve() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    env.ledger().with_mut(|l| l.timestamp = 1500);
    client.cast_attestor_vote(&attestors.get(0).unwrap(), &id, &CommitmentStatus::Fulfilled);
    client.cast_attestor_vote(&attestors.get(1).unwrap(), &id, &CommitmentStatus::Fulfilled);
    client.cast_attestor_vote(&attestors.get(2).unwrap(), &id, &CommitmentStatus::Late);
    client.cast_attestor_vote(&attestors.get(3).unwrap(), &id, &CommitmentStatus::Breached);

    let commitment = client.get_commitment(&id);
    assert_eq!(commitment.status, CommitmentStatus::Pending);

    let tally = client.get_vote_tally(&id);
    assert_eq!(tally.fulfilled, 2);
    assert_eq!(tally.late, 1);
    assert_eq!(tally.breached, 1);
}

#[test]
fn test_cast_attestor_vote_fails_for_non_attestor() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    let stranger = Address::generate(&env);
    let res = client.try_cast_attestor_vote(&stranger, &id, &CommitmentStatus::Fulfilled);
    assert_eq!(res, Err(Ok(Error::NotAttestor.into())));

    let res2 = client.try_cast_attestor_vote(&issuer, &id, &CommitmentStatus::Fulfilled);
    assert_eq!(res2, Err(Ok(Error::NotAttestor.into())));
}

#[test]
fn test_cast_attestor_vote_twice_fails() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    let res = client.try_cast_attestor_vote(&a1, &id, &CommitmentStatus::Breached);
    assert_eq!(res, Err(Ok(Error::AlreadyVoted.into())));
}

#[test]
fn test_cast_attestor_vote_fails_for_invalid_outcome() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    let a1 = attestors.get(0).unwrap();
    let res = client.try_cast_attestor_vote(&a1, &id, &CommitmentStatus::Pending);
    assert_eq!(res, Err(Ok(Error::InvalidOutcome.into())));

    let res2 = client.try_cast_attestor_vote(&a1, &id, &CommitmentStatus::Disputed);
    assert_eq!(res2, Err(Ok(Error::InvalidOutcome.into())));
}
...
fn test_cast_attestor_vote_fails_on_regular_commitment() {
    ...
    let res = client.try_cast_attestor_vote(&issuer, &id, &CommitmentStatus::Fulfilled);
    assert_eq!(res, Err(Ok(Error::NotAttestor.into())));
}

#[test]
fn test_cast_attestor_vote_fails_after_resolution() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    env.ledger().with_mut(|l| l.timestamp = 1500);
    for i in 0..3 {
        client.cast_attestor_vote(&attestors.get(i).unwrap(), &id, &CommitmentStatus::Fulfilled);
    }

    // A non-voting attestor tries to vote after resolution.
    let res = client.try_cast_attestor_vote(&attestors.get(4).unwrap(), &id, &CommitmentStatus::Fulfilled);
    assert_eq!(res, Err(Ok(Error::AlreadyResolved.into())));
}
...
fn test_vote_after_deadline_fails() {
    ...
    env.ledger().with_mut(|l| l.timestamp = 2000 + ATTESTOR_VOTE_TIMEOUT_SECONDS + 1);
    let a1 = attestors.get(0).unwrap();
    let res = client.try_cast_attestor_vote(&a1, &id, &CommitmentStatus::Fulfilled);
    assert_eq!(res, Err(Ok(Error::VotingClosed.into())));
}

#[test]
fn test_finalize_commitment_after_timeout_sets_fallback() {
    let (env, client, issuer, _counterparty, attestors) = setup_voting_test();

    env.ledger().with_mut(|l| l.timestamp = 1000);
    let id = create_voting_commitment(&env, &client, &issuer, &_counterparty, &attestors, 2000);

    env.ledger().with_mut(|l| l.timestamp = 1500);
    client.cast_attestor_vote(&attestors.get(0).unwrap(), &id, &CommitmentStatus::Fulfilled);

    assert_eq!(client.can_finalize_commitment(&id), false);

    env.ledger().with_mut(|l| l.timestamp = 2000 + ATTESTOR_VOTE_TIMEOUT_SECONDS + 1);
    assert_eq!(client.can_finalize_commitment(&id), true);

    // Callable by anyone to unblock the stalled commitment.
    client.finalize_commitment(&id);

    let commitment = client.get_commitment(&id);
    assert_eq!(commitment.status, CommitmentStatus::Breached);
    assert_eq!(commitment.attested_at, Some(2000 + ATTESTOR_VOTE_TIMEOUT_SECONDS + 1));
}

#[test]
fn test_finalize_commitment_before_deadline_fails() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    env.ledger().with_mut(|l| l.timestamp = 2500);
    let res = client.try_finalize_commitment(&id);
    assert_eq!(res, Err(Ok(Error::VotesNotMet.into())));
}

#[test]
fn test_finalize_commitment_fails_on_regular_commitment() {
    let (env, client, issuer, counterparty) = setup_test();
...
#[test]
fn test_finalize_commitment_fails_after_resolution() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
...
#[test]
fn test_finalize_commitment_updates_reputation() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
...
#[test]
fn test_attestor_vote_updates_reputation_on_resolution() {
    let (env, client, issuer, counterparty, attestors) = setup_voting_test();
    ...
    env.ledger().with_mut(|l| l.timestamp = 1500);
    for i in 0..3 {
        client.cast_attestor_vote(&attestors.get(i).unwrap(), &id, &CommitmentStatus::Late);
    }
...
#[test]
fn test_get_vote_tally_fails_for_nonexistent_id() {
    let (_env, client, _issuer, _counterparty, _attestors) = setup_voting_test();
    let res = client.try_get_vote_tally(&999);
    assert_eq!(res, Err(Ok(Error::CommitmentNotFound.into())));
}

#[test]
fn test_attest_fails_for_m_of_n_commitment() {
    let (env, client, issuer, counterparty, _attestors) = setup_voting_test();

    env.ledger().with_mut(|l| l.timestamp = 1000);
    let id = create_voting_commitment(
        &env,
        &client,
        &issuer,
        &counterparty,
    ...
    // Issuer/counterparty cannot unilaterally bypass the required consensus.
...
#[test]
#[should_panic]
fn test_attestor_vote_requires_auth() {
    let env = Env::default();
    let contract_id = env.register(RegistryContract, ());
    let client = RegistryContractClient::new(&env, &contract_id);
    let issuer = Address::generate(&env);
...
#[test]
fn test_attestor_vote_events_emitted() {
    use soroban_sdk::testutils::Events;
    ...
    let expected_topics: Vec<Val> = (symbol_short!("voted"), 1u64, a1.clone()).into_val(&env);
    assert_eq!(last.0, client.address);
    assert_eq!(last.1, expected_topics);
}

#[test]
fn test_resolution_and_fallback_events_emitted() {
    use soroban_sdk::testutils::Events;
    use soroban_sdk::{symbol_short, FromVal, IntoVal, Val, Vec};
    ...
    let all_events = env.events().all();
    let resolved = all_events.get(all_events.len() - 1).unwrap();
    let expected_resolved_topics: Vec<Val> = (symbol_short!("vresolved"), 1u64).into_val(&env);
    assert_eq!(resolved.0, client.address);
    assert_eq!(resolved.1, expected_resolved_topics);
    assert_eq!(
        CommitmentStatus::from_val(&env, &resolved.2),
        CommitmentStatus::Fulfilled
    );

    // A second commitment for the fallback event check.
    let id2 = create_voting_commitment(&env, &client, &issuer, &counterparty, &attestors, 4000);
    env.ledger().with_mut(|l| l.timestamp = 4000 + ATTESTOR_VOTE_TIMEOUT_SECONDS + 1);
    client.finalize_commitment(&id2);

    let all_events = env.events().all();
    let fallback = all_events.get(all_events.len() - 1).unwrap();
    let expected_fallback_topics: Vec<Val> = (symbol_short!("fallback"), 2u64).into_val(&env);
    assert_eq!(fallback.0, client.address);
    assert_eq!(fallback.1, expected_fallback_topics);
    assert_eq!(
