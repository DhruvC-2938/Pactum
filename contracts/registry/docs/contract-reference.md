...
- `dispute()` requires `attested_at` to compute the window deadline (`deadline = attested_at + DISPUTE_WINDOW_SECONDS`); without it, the call panics with `Error::InvalidTransition`. Once cleared, a resolved commitment has no anchor from which to open a dispute window, so re-disputing it is _structurally_ impossible — no party, and not even the arbitrator, can route it back into `Disputed`.
...
- **Note**: An _archived_ entry is not readable as absent — touching one aborts the invocation. Restore such keys with `RestoreFootprint` before migrating them.
