...
export const merkleProofNodeSchema = z
  .object({
...
export const headerProofSchema = z
  .object({
    previousLedgerHash: z
      .string()
      .regex(hex32Regex, 'previousLedgerHash must be a 32-byte hex string'),
...
export const pactumStateProofSchema = z
  .object({
...
export const batchedProofEntrySchema = z
  .object({
    sequenceId: z.number().int().min(0),
    stellarAddress: z.string().min(1),
    scoreData: scoreDataSchema,
...
export const pactumBatchedStateProofSchema = z
  .object({
