...
export class WebRTCProvider {
  ...
  private ensurePeerConnection(peerId: string): RTCPeerConnection {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        this.signaling?.send({ from: this.peerId, to: peerId, type: 'ice', payload: event.candidate.toJSON() })
      }
    }
  ...
  private wireDataChannel(peerId: string, channel: RTCDataChannel): void {
    ...
    channel.addEventListener('open', () => {
      ...
      const session = new SignedPeerSession(this.doc, this.identity, this.attestation, channelToPeerLink(channel), {
