...
import { attestSessionKey, createSessionIdentity, verifyAttestation, type WalletSigner } from './signing'
...
describe('session key attestation', () => {
  ...
  it('rejects an attestation bound to a session key it was not signed for', async () => {
    ...
    expect(verifyAttestation({ ...attestation, sessionPublicKeyRaw: otherIdentity.publicKeyRaw })).toBe(false)
  })
