...
/**
 * Verifies a zero-trust PactumStateProof.
 *
 * @param proof - The PactumStateProof object to verify.
 * @param trustedLedgerHeaderHash - Optional known Stellar block/ledger hash to verify against.
...
export function verifyPactumStateProof(
  ...
  try {
    ...
    const expectedLeafHex = bytesToHex(expectedLeaf);
    ...
    if (computedRootHex.toLowerCase() !== proof.stateRootHash.toLowerCase()) {
      return {
        valid: false,
    ...
    const computedHeaderHex = bytesToHex(computedHeader);
